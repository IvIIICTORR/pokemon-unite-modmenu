...
void Init(JNIEnv *env, jobject thiz, jobject ctx, jobject title, jobject subtitle) {
    //Set sub title
    setText(env, title, OBFUSCATE("<b>Pokemon Unite - Map Hack</b>"));

    //Set sub title
    setText(env, subtitle, OBFUSCATE("<b><marquee><p style=\"font-size:30\">"
                                     "<p style=\"color:green;\">Modded by Victor</p> | "
                                     "Pokemon Unite Map Hack v1.0</p>"
                                     "</marquee></b>"));
    ...
    //Toast Example
    Toast(env, ctx, OBFUSCATE("Modded by Victor"), ToastLength::LENGTH_LONG);
}
